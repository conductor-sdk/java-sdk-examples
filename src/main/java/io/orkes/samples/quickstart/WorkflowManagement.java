...
public class WorkflowManagement {
    ...
    private CompletableFuture<Workflow> executeWorkflow(String name, Integer version, Map<String, Object> input) {
    ...
    private void startWorkers() {
        ...
        TaskRunnerConfigurer.Builder builder = new TaskRunnerConfigurer.Builder(taskClient, List.of(new HelloWorld()));
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(StringUtils.isBlank(key) || StringUtils.isBlank(secret)) {
            log.warn("\n\nMissing KEY and|or SECRET.  Attemping to connect to " + conductorServer + " without authentication\n\n");
        ...
        CompletableFuture<Workflow> future = workflowManagement.executeWorkflow(workflowDef.getName(), workflowDef.getVersion(), input);
        future.thenAccept(workflow -> {
            try {
                String formattedOutput = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(workflow.getOutput());
                log.info("\n\n\n\nCompleted execution.  Workflow Id {} \n with output \n{} \n\n", workflow.getWorkflowId(), formattedOutput);
